	...
	responseSchema: {
		...
		properties: {
			categoryName: {
				...
				description: "The name of the category to route to.",
...
async function run() {
	...
	const userQuery =
		"I have been charged twice on my credit card for a single purchase.";
	//const userQuery =
	...
	const routingPrompt = `
  Classify the following customer service query into one of the categories.
  
  Query: "${userQuery}"`;

	const routingResult = await chatSession.sendMessage(routingPrompt);
	//const category = routingResult.response.text().trim();
	console.log("Category:", JSON.stringify(routingResult, null, 2));
	...
	const specializedResult = await chatSession2.sendMessage(specializedPrompt);
